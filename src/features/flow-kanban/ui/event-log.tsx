...
export function EventLog({ events }: { events: FlowEvent[] }) {
  return (
    <section className="rounded-md border border-slate-200 bg-white p-4 dark:border-slate-700 dark:bg-slate-900">
      <h2 className="m-0 mb-3 text-left text-base font-semibold text-slate-950 dark:text-slate-50">WebSocketイベント想定ログ</h2>
      <div className="max-h-64 space-y-2 overflow-auto">
        {events.length === 0 ? (
          ...
          events.map((event) => (
            <article key={event.id} className="rounded-md bg-slate-50 px-3 py-2 text-left dark:bg-slate-800">
